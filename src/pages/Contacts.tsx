    ...
    <div className="flex flex-col w-full bg-page-bg">
      ...
      <section className="border-b-2 border-border-main bg-surface-main p-6 md:p-12 lg:p-16 relative overflow-hidden">
        <div className="max-w-[1000px] z-10 relative">
          ...
          <p className="mt-8 text-xl max-w-2xl font-light leading-relaxed text-text-main">
            Зв'яжіться з нами для обговорення академічних програм, партнерств або спільних проєктів.
          </p>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 divide-y-2 lg:divide-y-0 lg:divide-x-2 divide-border-main border-b-2 border-border-main">
        ...
        <div className="lg:col-span-7 bg-page-bg flex flex-col divide-y-2 divide-border-main">
          <div className="grid grid-cols-1 md:grid-cols-2 divide-y-2 md:divide-y-0 md:divide-x-2 divide-border-main">
            <div className="p-6 md:p-12">
              <span className="font-mono text-xs text-text-dim block mb-6 uppercase tracking-widest">Адреса</span>
              <address className="not-italic text-lg md:text-xl font-medium leading-relaxed uppercase opacity-90">
                М. Львів,<br />
                Вул. Січових Стрільців, 7<br />
                Аудиторія 204
              </address>
              <div className="mt-8">
                <a href="https://maps.google.com" target="_blank" rel="noopener noreferrer" className="font-mono text-xs uppercase tracking-widest font-bold hover:underline underline-offset-4 flex items-center gap-2">
                  Відкрити в Google Maps <ArrowRight className="w-4 h-4" />
                </a>
              </div>
            </div>
            
            <div className="p-6 md:p-12 bg-surface-main">
              <span className="font-mono text-xs text-text-dim block mb-6 uppercase tracking-widest">Зв'язок</span>
              <div className="space-y-6 text-lg md:text-xl font-medium uppercase break-all">
                ...
                <div>
                  <span className="block font-mono text-[10px] text-text-dim mb-1">ТЕЛЕФОН:</span>
        ...
        <div className="lg:col-span-5 bg-surface-main p-6 md:p-12 border-t-2 lg:border-t-0 border-border-main">
          <span className="font-mono text-xs text-text-dim block mb-8 uppercase tracking-widest">Форма</span>
          <h2 className="text-3xl font-bold uppercase tracking-tight mb-8 leading-none">Напишіть нам</h2>
          
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div>
              <label htmlFor="name" className="font-mono text-xs uppercase tracking-widest block mb-2 font-bold opacity-80">Ім'я</label>
              <input 
                type="text" 
                id="name"
                name="name"
            ...
            <div>
              <label htmlFor="message" className="font-mono text-xs uppercase tracking-widest block mb-2 font-bold opacity-80">Повідомлення</label>
              <textarea 
                id="message"
                name="message"
                rows={5}
            ...
            <button 
              ...
              className="w-full bg-text-main text-page-bg font-bold uppercase tracking-widest py-5 px-6 transition-colors focus-ring outline-none enabled:hover:opacity-80 active:scale-[0.98] transform duration-150 mt-4 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {formState === "submitting" ? "Обробка..." : formState === "success" ? "Надіслано!" : formState === "error" ? "Помилка (Спробуйте знов)" : "Відправити"}
            </button>
            <p className="font-mono text-[10px] text-text-dim mt-4 uppercase">
              * Дані будуть оброблені за Київським часом.
            </p>
          </form>
        </div>
